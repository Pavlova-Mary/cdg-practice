...
describe "greetings" do
  it 'should do ok for age equal or bigger than 18' do
    ...
    expect(helloMaker).to eql("Привет, Иван Соболев. Самое время заняться делом!")
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Владимир', 'Громов', 20)
    expect(helloMaker).to eql("Привет, Владимир Громов. Самое время заняться делом!")
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Василий', 'Андреев', 43)
    expect(helloMaker).to eql("Привет, Василий Андреев. Самое время заняться делом!")
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Снежанна', 'Лебедева', 18)
    expect(helloMaker).to eql("Привет, Снежанна Лебедева. Самое время заняться делом!")
  end

  it 'should do ok for age smaller than 18 ' do
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Максим', 'Моисеев', 14)
    expect(helloMaker).to eql("Привет, Максим Моисеев.Тебе меньше 18 лет, но начать учиться программировать никогда не рано")
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Денис', 'Григорьев', 15)
    expect(helloMaker).to eql("Привет, Денис Григорьев.Тебе меньше 18 лет, но начать учиться программировать никогда не рано")
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Пётр', 'Мартынов', 16)
    expect(helloMaker).to eql("Привет, Пётр Мартынов.Тебе меньше 18 лет, но начать учиться программировать никогда не рано")
    allow_any_instance_of(Kernel).to receive(:gets).and_return('Светлана', 'Орехова', 17)
    expect(helloMaker).to eql("Привет, Светлана Орехова.Тебе меньше 18 лет, но начать учиться программировать никогда не рано")
  end
end
